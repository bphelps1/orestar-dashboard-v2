-- Explore's browse call, returning each row's ORESTAR sub-type ("In-Kind
-- Contribution", "Personal Expenditure for Reimbursement", ...) and filtering on
-- it. Otherwise identical to search_transactions as 031 defines it.
--
-- A new name rather than a new search_transactions signature: 010, 011 and 031
-- re-create the old signature on every apply, and two overloads whose trailing
-- parameters all have defaults make PostgREST refuse to pick either one, so
-- Explore would break for the length of every migration run. The old function
-- stays for pages still cached with the previous explore.js.
-- Apply 033 first so the sub-type filter is indexed.
create or replace function explore_transactions(
  p_filer      text    default '',
  p_payee      text    default '',
  p_donor_id   text    default '',
  p_tran_type  text    default '',
  p_sub_type   text    default '',
...
begin
  ...
  v_sort := case p_sort
    ...
    when 'tran_type'                   then 'tran_type'
    when 'sub_type'                    then 'sub_type'
  ...
  return query execute format($q$
    select t.tran_date, t.tran_type, t.sub_type, t.amount,
    ...
    where ($1 = '' or t.filer_canonical ilike '%%' || $1 || '%%'
      ...
      and ($5  = '' or t.sub_type = $5)
      and ($6  = '' or t.book_type ilike '%%' || $6 || '%%')
      and ($7 is null or t.tran_date >= $7)
      and ($8 is null or t.tran_date <= $8)
      and ($9 is null or t.amount >= $9)
      and ($10 is null or t.amount <= $10)
    order by %I %s nulls last, t.tran_id desc
    limit $11 offset $12
  $q$, v_sort, v_dir)
  using coalesce(p_filer, ''), coalesce(p_payee, ''), v_donor_ids,
        coalesce(p_tran_type, ''), coalesce(p_sub_type, ''), coalesce(p_book_type, ''),
        p_date_from, p_date_to, p_amt_min, p_amt_max,
        greatest(1, least(p_limit, 1000)), greatest(0, p_offset);
end;
$$;

grant execute on function explore_transactions(
  text, text, text, text, text, text, date, date, numeric, numeric, text, boolean, int, int
